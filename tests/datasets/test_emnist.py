...
def base_dataset(dataset_root):
    ...
    logger.info(
        "Initializing base EMNIST dataset (this will download data if needed)..."
    )
...
def test_from_path(dataset_root):
    ...
    dataset = EMNISTDataset.from_path(
        str(dataset_root), download=False
    )  # Reuse existing data
...
def test_label_mapping_creation(dataset_root):
    ...
    mapping_path = os.path.join(
        dataset_root, "EMNIST", "raw", "emnist-bymerge-mapping.txt"
    )
