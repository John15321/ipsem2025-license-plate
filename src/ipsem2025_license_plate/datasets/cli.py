...
def info_command(
    ...
    for label, count in sorted(class_counts.items()):
        ...
        table.add_row(
            class_mapping.get(label, str(label)),
            str(count),
            f"{percentage:.1f}%"
        )
...
def preview_command(
    ...
    save_plot: Optional[str] = typer.Option(
        ...
        "--save-plot",
        "-s",
        help="Save preview to file instead of displaying"
    ),
    verbose: bool = typer.Option(
        False,
        "--verbose",
...
def validate_command(
    ...
    if not path_obj.exists():
        console.print(f"[bold red]Dataset path does not exist:[/bold red] {dataset_path}")
        return 1
    
    if not path_obj.is_dir():
        console.print(f"[bold red]Dataset path is not a directory:[/bold red] {dataset_path}")
        return 1
    
    # Check for class directories
    class_dirs = [d for d in path_obj.iterdir() if d.is_dir()]
    if not class_dirs:
        console.print(f"[bold red]No class directories found in[/bold red] {dataset_path}")
        return 1
    ...
    with Progress(
        ...
        task = progress.add_task("Validating class directories...", total=len(class_dirs))
    ...
    if issues_found:
        ...
        console.print("[bold green]Dataset validation successful: no issues found[/bold green]")
